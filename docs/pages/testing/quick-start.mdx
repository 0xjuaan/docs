...
Then, in your test file, import `CoreSimulatorLib` as shown in the template, and call `CoreSimulatorLib.init()` in the `setUp()`.
...
function setVaultMultiplier(address vault, uint256 multiplier) internal;

// simulate staking yield
function setStakingYieldIndex(uint256 multiplier) internal;
