...
**Usage through PrecompileLib:** The `PrecompileLib` exposes a [getTokenIndex function](https://github.com/hyperliquid-dev/hyper-evm-lib/blob/b347756c392934712af9c27b92028a00b93cb68c/src/PrecompileLib.sol#L61-L66) to read from the `TokenRegistry`, and can be used instead of directly interacting with the `TokenRegistry` contract. 
...
Precompiles like `spotBalance`, `spotPx` and more, all require either a token index (for `spotBalance`) or a spot market index (for `spotPx`) as an input parameter. Similarly bridging an EVM token over to core requires knowing its token index
...
`TokenRegistry` solves this by providing a deployed onchain mapping from EVM contract addresses to their HyperCore token indexes, populated trustlessly using precompile lookups for each index.
