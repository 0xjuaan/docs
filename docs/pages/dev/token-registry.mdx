...
The `TokenRegistry` provides an onchain mapping between EVM token addresses and their corresponding HyperCore token and spot market indexes, populated trustlessly using precompile lookups for each index.
...
Calling HyperCore precompiles (for example, `spotBalance` and `spotPx`) requires token and spot market indexes, but those indexes can’t be derived from an EVM token address natively onchain. 
